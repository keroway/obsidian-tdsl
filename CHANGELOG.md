...
- **Nested-fence prefix round-trip** — format and lint fix strip the callout/list-indent prefix before parsing a nested `tdsl` block's body and restore it on every transformed line, instead of feeding the prefix to the parser or losing the indentation ([#251](https://github.com/keroway/obsidian-tdsl/issues/251))
