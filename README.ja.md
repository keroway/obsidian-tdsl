...
`now` は描画時の**年**に解決されるため、`unit year` と組み合わせて使ってください。
...
| `link` | 文字列（URL） | ツールチップに 1 行として表示される。**クリックはできない**（レンダラは `<a>` を出力しない） |
| `color` | 文字列（CSS 色） | 項目の塗り色を上書きする。レーン色や `color_map` より優先される |
